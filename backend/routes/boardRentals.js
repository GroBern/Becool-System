...
router.get('/', async (_req, res) => {
    try {
        const items = await BoardRental.find().sort({ createdAt: -1 });
        res.json(items);
    }
    catch (err) {
...
router.get('/:id', async (req, res) => {
    try {
        const item = await BoardRental.findById(req.params.id);
        if (!item)
...
router.post('/', async (req, res) => {
    try {
        ...
        const item = await BoardRental.create(data);
...
router.put('/:id', async (req, res) => {
    try {
        const item = await BoardRental.findByIdAndUpdate(req.params.id, req.body, { new: true, runValidators: true });
        if (!item)
            return res.status(404).json({ error: 'Not found' });
        res.json(item);
    }
...
router.delete('/:id', async (req, res) => {
    try {
        const item = await BoardRental.findByIdAndDelete(req.params.id);
        if (!item)
            return res.status(404).json({ error: 'Not found' });
...
router.post('/:id/payments', async (req, res) => {
    try {
        ...
        const rental = await BoardRental.findById(req.params.id);
        if (!rental)
            return res.status(404).json({ error: 'Not found' });
